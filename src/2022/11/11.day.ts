...
export default class Day11 {
	...
	get_level_of_business_10000_rounds() {
		...
		const denominator = monkeys
			.map((monkey) => monkey.divideBy)
			.reduce((total, value) => total * value);

		for (let round = 0; round < 10_000; round++) {
			monkeys.forEach((monkey, index, arrayMonkeys) => {
				this.doAMonkeyComplexRound(
					arrayMonkeys[index],
					arrayMonkeys,
					denominator,
				);
		...
		const result = monkeys
			...
			.reduce((total, value) => total * value);
		console.log(
			`Day 11 => level of monkey business after 10000 rounds`,
			result,
		);
	}
	...
	doAMonkeyComplexRound(
		monkey: Monkey,
		arrayOfMonkeys: Array<Monkey>,
		denominator: bigint,
	) {
		monkey.items.forEach((item, index, itemsArray) => {
