...
export default class Day8 {
	...
	get_highest_scenic_score() {
		...
		const totalResult = Math.max(...scenicScores);
		console.assert(totalResult === 392080);
		console.log("DAY 8 => best scenic score", totalResult);
	}
	...
	checkIfTreeIsVisible(
		...
		return (
			directions.filter(
				(direction) =>
					direction.find((otherTree) => otherTree >= treeHeight) === undefined,
			).length > 0
		);
	}
