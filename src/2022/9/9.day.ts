...
export default class Day9 {
    ...
    find_number_of_visited_position_by_the_tail() {
        ...
        console.log('DAY 9 => number of visited position by the tail for a body of 2', rope.tailPath.size);
        console.log('DAY 9 => number of visited position by the tail for a body of 10', ropePart2.tailPath.size);
    }
    ...
    interpretCommandString(commandStr: string, rope: Rope): Rope {
        ...
        for (let index = 0; index < nbSteps; index++) {
            for (let knotIndex = 0; knotIndex < rope.knots.length; knotIndex++) {
                ...
                const id = `${rope.knots[rope.knots.length - 1].row};${rope.knots[rope.knots.length - 1].column}`;
    ...
    isKnotCloseToPrevious(rope: Rope, knotIndex: number): boolean {
        const previousKnot = rope.knots[knotIndex - 1];
        const currentKnot = rope.knots[knotIndex];
        return Math.abs(currentKnot.row - previousKnot.row) <= 1 && Math.abs(currentKnot.column - previousKnot.column) <= 1;
    }
    ...
    visualize(rope: Rope, name: string = 'visualize') {
        ...
        for (let x = 0; x < nbRow; x++) {
            for (let y = 0; y < nbCol; y++) {
                const value = rope.tailPath.has(`${x};${y}`) ? rope.tailPath.get(`${x};${y}`) : '.';
                if (grid[x]) {
                    grid[x].push(value)
                } else {
