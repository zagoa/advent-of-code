...
export default class Day6 {
    ...
    find_when_message_begin() {
        ...
        const SEQ_LENGTH = 4;
        const startSequenceIndex = this.findSequenceIndexOfUniqueChar(data, SEQ_LENGTH)
        console.log('DAY 6 => find index when the message begin', startSequenceIndex + SEQ_LENGTH)
    }
    ...
    find_when_message_begin_with_14th_sequence() {
        ...
        const SEQ_LENGTH = 14;
        const startSequenceIndex = this.findSequenceIndexOfUniqueChar(data, SEQ_LENGTH)
        console.log('DAY 6 => find index when the message begin (with 14th different char)', startSequenceIndex + SEQ_LENGTH)
    }

    findSequenceIndexOfUniqueChar(text: string, sequenceLength: number) {
        for (let index = 0; index < text.length - 1; index++) {
            const sequenceMap = new Map();
            for (let seqIndex = index; seqIndex < index + sequenceLength; seqIndex++) {
