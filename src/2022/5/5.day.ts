...
export default class Day5 {
    ...
    find_crates_on_top_with_crate_mover_9001() {
        ...
        console.log("DAY 5 => find crates on top with 9001", this.getTopLetters(warehouse));
    }


    applyCommands(warehouse: Array<Array<string>>, commands: Array<Command>, crateMoverType: '9000' | '9001' = '9000') {
        commands.forEach(command => {
            const removed = warehouse[command.from].splice(warehouse[command.from].length - command.move, command.move)
    ...
    parseInput(): { crates: Array<string>, commandsAsString: Array<string> } {
        ...
        return {crates: data.slice(0, splitIndex), commandsAsString: data.slice(splitIndex + 1)}
    }
    ...
    createWarehouse(cratesAsString: Array<string>): Array<Array<string>> {
        ...
        const numberOfCratesByLine: number = parseInt(lastLineDefiningWarehouse[lastLineDefiningWarehouse.length - 1]);

        for (let lineNumber = numberOfCratesByLine - 2; lineNumber > -1; lineNumber = lineNumber - 1) {
    ...
    parseCommands(commandsAsString: Array<string>): Array<Command> {
        ...
        commandsAsString.forEach(commandStr => {
            ...
            const command = new Command(parseInt(commandStrSplit[1]), parseInt(commandStrSplit[3]) - 1, parseInt(commandStrSplit[5]) - 1);
