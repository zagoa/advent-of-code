...
export default class Day12 {
    ...
    get_shortest_path_by_choosing_starting_point() {
        ...
        fs.writeFileSync('results', JSON.stringify(results.filter(item => squares.get(item.id).value === 'a')));
        ...
        console.log(
            `Day 12 => shortest path length for a starting point chosen ${Math.min(
                ...results.filter(item => squares.get(item.id).value === 'a').map(item => item.distance),
            )} in ${Date.now() - start}ms`,
        );
    }

    dijkstraImplementation(start: Square, squares: Map<string, Square>): Map<string, { id: string; distance: number; visited: boolean; from?: Array<string> }> {
