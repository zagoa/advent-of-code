...
export default class Day7 {
	...
	total_size_of_those_directories() {
		...
		console.log(
			"DAY 7 => total size of directories under 100000",
			this.computeGlobalSizeUnder(mapOfDirectory, 100000),
		);
	}
	...
	find_directory_to_delete() {
		...
		console.log(
			`DAY 7 => to free ${spaceToFree} you need to delete`,
			this.getMinFolderToFreeEnoughSpace(spaceToFree, mapOfDirectory).totalSize,
		);
	}
		...
		for (let item of data) {
			...
			if (item.startsWith("$")) {
				...
				currentDir.files.push(
					new FileInfo(lineSplit[1], parseInt(lineSplit[0])),
				);
			}
	...
	private computeTotalSizeOfDirectories(
		mapOfDirectory: Map<string, DirectoryInfo>,
	) {
		mapOfDirectory.forEach((directory, key) => {
			directory.totalSize = this.recursivelyComputeSize(
				directory,
				mapOfDirectory,
			);
		});
	...
	private recursivelyComputeSize(
		...
		directoryInfo.dirs.forEach((dir) => {
			if (mapOfDirectory.has(this.buildNewPath(directoryInfo.path, dir))) {
				const directory = mapOfDirectory.get(
					this.buildNewPath(directoryInfo.path, dir),
				);
				previousSize = this.recursivelyComputeSize(
					directory,
					mapOfDirectory,
					previousSize,
				);
	...
	private computeGlobalSizeUnder(
		mapOfDirectory: Map<string, DirectoryInfo>,
		limit: number,
	) {
		let globalSize = 0;
	...
	private getMinFolderToFreeEnoughSpace(
		spaceToFree: number,
		mapOfDirectory: Map<string, DirectoryInfo>,
	): DirectoryInfo {
		...
		mapOfDirectory.forEach((value) => {
			if (
				value.totalSize >= spaceToFree &&
				(!bestCandidate.totalSize || value.totalSize < bestCandidate.totalSize)
			) {
				bestCandidate = value;
			}
		});
