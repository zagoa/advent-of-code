...
const REQUIERED_FIELD = [
    {'field': 'byr', requiered: true, isValid: x => numberBetweenRange(x, 4, 1920, 2002)},
    {'field': 'iyr', requiered: true, isValid: x => numberBetweenRange(x, 4, 2010, 2020)},
    {'field': 'eyr', requiered: true, isValid: x => numberBetweenRange(x, 4, 2020, 2030)},
    {'field': 'hgt', requiered: true, isValid: height => heightIsValid(height)},
    {'field': 'ecl', requiered: true, isValid: eye => !!EYE_COLOR.find(item => item === eye)},
    {
        'field': 'hcl',
        requiered: true,
        isValid: hairColor => hairColor.length === 7 && RegExp(/#(\d|[a-f]){6}/).test(hairColor)
    },
    {'field': 'pid', requiered: true, isValid: pid => pid.length === 9 && RegExp(/\d{9}/).test(pid)},
...
function countValidPassports(inputs: Array<string>, checkCondition: boolean = false): number {
    return inputs.reduce((count: number, input: string) => {
        ...
        const remainingFields = REQUIERED_FIELD.filter(field =>
        ...
        if (remainingFields.length === 0 || (remainingFields.length === 1 && !remainingFields[0].requiered)) {
            count++;
        }
        return count;
    }, 0);
...
function numberBetweenRange(input: number | string, size: number, min: number, max: number): boolean {
    if (!isNaN(size)) {
